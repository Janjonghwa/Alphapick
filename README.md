...
새 PC에서 실행 전 준비물
당연하지만, 새 PC에도 아래 핵심 런타임이 설치되어 있어야 명령어가 실행됩니다.

Python 3.10 ~ 3.12 설치 (환경변수 PATH 등록 필수)
Node.js (LTS 버전 권장) 설치 (npm 명령어 사용을 위해 필요)
...
백엔드 환경 변수는 프로젝트 루트의 `.env` 하나에서 관리합니다. 새 환경을 만들 때는 `.env.example`을 복사해 값을 채웁니다.
