<template>
  <section class="page-shell flex min-h-[calc(100vh-65px)] items-center justify-center py-8">
    <form class="panel w-full max-w-md p-6" @submit.prevent="submit">
      <h1 class="text-3xl font-black">로그인</h1>
      <label class="mt-6 block text-sm font-bold">아이디</label>
      <input v-model="form.username" class="field mt-2" autocomplete="username" required />
      <label class="mt-4 block text-sm font-bold">비밀번호</label>
      <input v-model="form.password" class="field mt-2" type="password" autocomplete="current-password" required />
