...
const memeComment = computed(() => {
  ...
  if (hasWarning) {
    return {
      ...
      details: [`💪 강점: ${strongestDetail(score)}`, `⚠️ 약점: ${score.warning || "경고 신호, 가즈아 잠시 보류"}`, `🎯 행동: ${action} · 보유자는 리스크 관리`],
    };
  }
  if (timing <= 0) {
    return {
      headline: "풀매수 버튼 완전 압수",
      details: [`💪 강점: ${strongestDetail(score)}`, "⚠️ 약점: 타이밍 0점, 진입각 실종", `🎯 행동: ${action} · 보유자는 리스크 관리`],
    };
  }
  if (company >= 70 && timing < 60) {
    return {
      headline: isOverheated ? "로켓은 발사, 지금 타면 멀미각" : "본체는 국밥, 진입각은 품절",
      details: [`💪 강점: 퀄리티 ${formatScore(company)}점, 본체 체력 확실`, `⚠️ 약점: 타이밍 ${formatScore(timing)}점, 추격매수 조심`, `🎯 행동: ${action} · 보유자는 익절각 점검`],
    };
  }
  if (market >= 70 && company < 60) {
    return {
      headline: "차트는 황제주, 본체는 점검 중",
      details: [`💪 강점: 시장 ${formatScore(market)}점, 떡상 폼 확인`, `⚠️ 약점: 퀄리티 ${formatScore(company)}점, 존버 체력 부족`, `🎯 행동: ${action} · 장기 몰빵은 보류`],
    };
  }
  if (company >= 70 && market >= 70 && timing >= 70) {
    return {
      headline: "본체·차트·타점 전부 로그인",
      details: ["💪 강점: 회사와 시장 점수 모두 상위권", `⚠️ 약점: ${isHighValuation ? "몸값 부담은 우주여행 중" : "과열 부담도 아직 제한적"}`, `🎯 행동: ${action} · 보유자는 추세 존버`],
    };
  }
  return {
    headline: isHighValuation ? "본체는 국밥, 가격은 파인다이닝" : "가즈아 전에 타이밍 확인",
    details: [`💪 강점: ${strongestDetail(score)}`, `⚠️ 약점: ${weakestDetail(score)}`, `🎯 행동: ${action} · 보유자는 신호 점검`],
  };
});
